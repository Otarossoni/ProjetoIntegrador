...
import PromocaoExpiradaList from "./PromocaoExpiradaList";
import PromocaoExpiradaForm from "./PromocaoExpiradaForm";
import PromocaoExpiradaSrv from "./PromocaoExpiradaSrv";
...
function PromocaoCon() {
    ...
    PromocaoExpiradaSrv.listar()
      .then((response) => {
        ...
        toastRef.current.show({
          severity: "success",
          summary: "Promoções expiradas atualizadas!",
          life: 3000,
        });
      })
      .catch((e) => {
  ...
  const salvar = () => {
    ...
    if (promocao._id == null) {
      PromocaoExpiradaSrv.incluir(promocao)
        .then((response) => {
          setEditando(false);
          onClickAtualizar();
          toastRef.current.show({
      ...
      // alteração
      PromocaoExpiradaSrv.alterar(promocao)
        .then((response) => {
          setEditando(false);
          onClickAtualizar();
          toastRef.current.show({
  ...
  const excluirConfirm = (_id) => {
    PromocaoExpiradaSrv.excluir(_id)
      .then((response) => {
        onClickAtualizar();
        toastRef.current.show({
          severity: "success",
