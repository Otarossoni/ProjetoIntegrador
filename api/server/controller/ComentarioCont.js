...
module.exports = {
    listar: async (req, res) => {
        ...
        }).populate('promocao_id').sort({ nome: 1 }); // -1 decrescente 1 crescente
    },
    ...
    filtrar: (req, res) => {
        Comentario.find(
          {
            $or: [
              { comentario_titulo: { $regex: req.params.filtro, $options: "i" } },
              { comentario_descricao: { $regex: req.params.filtro, $options: "i" } }
            ],
          },
          function (err, obj) {
            err ? res.status(400).send(err) : res.status(200).json(obj);
